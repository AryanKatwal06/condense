...
- **107 builtin definitions** in `filters/index.toml` (31 Java-backed + 76 leftover). **~140 registered command prefixes** across Java `@CommandFilter` annotations and leftover `commands =` rows (several definitions claim multiple prefixes, e.g. `CargoInstallFilter` → `cargo install` + `cargo build`; `GrepFilter` → `grep` + `rg`; leftover terraform/tofu plan/apply/destroy/init/validate/fmt/`state list`; leftover `dotnet format`, `msbuild`, `dotnet msbuild`; leftover `biome check` / `nx run` / `swift build`). **`CatFilter` is `cat` only** — `read` is the Phase 10 subcommand, not a proxied prefix. Bare `terraform state` / `tofu state` / `systemctl` are not claimed. The pinned zap-family inventory is `inventory/zap-families.json` at zap `d9498bb`.
...
| Enhancement Phase 1 | **LANDED** | Virtual threads enabled by default on Java 21+ with graceful fallback and override controls; unverified competitor claims purged from documentation. |
