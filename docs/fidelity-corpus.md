...
There is no `condense replay` CLI. `NativeCorpusIT` writes `fixtures/pytest/typical.txt` next to a stub `pytest` (`pytest.cmd` on Windows) that prints that fixture and exits 1, prepends that directory to `PATH`, and runs the native binary as `condense pytest`. That path is now a classpath TOML pipeline inside the image. On Windows the native binary resolves PATHEXT shims itself because `ProcessBuilder` will not find `pytest.cmd` by bare name. `NativeCatalogMatrixIT` PATH-stubs every `filters/index.toml` definition from `inventory/native-catalog-matrix.json`. `NativeBuiltinDefinitionIT` proves schema v1 `config validate` (including a promoted `tail_lines` override) inside the same binary. `NativeTrustIT` proves an untrusted project override cannot drop `failed`, that `config trust --accept` can apply one, that `CONDENSE_TRUST_PROJECT_FILTERS` alone does not hatch, and that filtered stdout starts with `condense[filtered]`. Proof is the IT class names in `build.yml` native job logs. A missing native binary fails these ITs; it does not skip.

Filtered goldens include the Phase 6 provenance header. Inline `[[tests]]` do not go through `FilterResult` and stay unstamped.
