...
Condense versions several documents independently. This file is the policy. Numbers do not change in superiority Phase 3.

Additive optional fields are allowed within a major version. Unknown keys stay rejected on filter and IR documents. Unknown runtime stage names cannot instantiate a stage.
...
| IR / explain | `Document.SCHEMA_VERSION` = 1 | `JsonRenderer.parse` rejects; proxy keeps child exit and last-good text or opaque | Additive optional fields such as `termination` still parse |
