...
| Native soak & telemetry | Failsafe `NativeSoakIT` | N proxied successes with isolated dirs; tracks RSS memory slope, handle counts, temp cleanup | **Relative leak gate + zero temp leak.** Last-10 mean < 5× first-10 mean. Zero orphaned temp files. Handle delta < 100. Exports `target/soak-telemetry.json`. N=20 default; N=300 on main pushes; N=3000 on weekly scheduled workflow. |
