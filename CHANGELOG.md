...
- **MCP server.** `condense mcp --start` speaks JSON-RPC 2.0 on stdio (no HTTP, no new Maven dependency). Tools are `run`, `explain`, `read`, and `discover`. Resources are `condense://gain` and `condense://doctor`. `run` returns the schema-1 IR envelope; a non-zero child exit is not `isError`. MCP-supplied paths go through `ReadPathGate`. Bare `condense mcp` prints a client config snippet. Native proof is `NativeMcpIT`. See [docs/mcp.md](docs/mcp.md).
