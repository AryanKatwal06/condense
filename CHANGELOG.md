...
- **Catalog leftover dispatch.** Builtin `filters/*.toml` `commands` that no Java `@CommandFilter` claims are registered on `CatalogBackedFilter`. Nineteen leftover families ship as data only: mypy, dotnet test/build/restore, bundle install, rspec, rubocop, terraform plan/apply, helm install/upgrade/list, gh/glab list commands, curl/wget/psql, pnpm install, next build, prisma migrate/generate, playwright, prettier. Optional builtin-only `select_input` and `[gate]` (curl uses `passthrough_nonzero_exit`). Native proof is `NativeCatalogIT`. See [docs/filter-schema.md](docs/filter-schema.md).
