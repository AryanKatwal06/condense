...
### Fixed
- `NativeCatalogMatrixIT` Unix stubs print the fixture with `/bin/cat` (or a read loop), not bare `cat`. A stub named `cat` used to recurse on PATH until `Cannot fork`.
...
- SQLite open runs `PRAGMA integrity_check`. A failing check marks degraded, skips migrate, and keeps the proxy fail-open. Schema target stays 2. See [docs/durability.md](docs/durability.md).
...
- ESLint text IR now carries per-line findings (goldens unchanged). `IrRendererGoldenTest` locks TextRenderer+stamp for the whole corpus except `python-c/typical`. `NativeIrIT` covers eslint, docker ps, and opaque git status. `NativeHookIT` installs Hermes (`plugin.yaml` + `__init__.py`).
...
### Added / Improved
- **Agent integration security and verification.** Added conservative finite-state compound-command analyzer (`CompoundCommandAnalyzer`) to prevent permission and interception bypasses without rewriting commands. Defaults ambiguous syntax (unterminated quotes, subshells `$()`, backticks, unbalanced parentheses, raw newlines, redirects) to ask/deny; auto-allows only when every pipeline/compound segment passes. Embedded the finite-state tokenizer across all 12 hook templates (Claude Code, Cursor, Gemini, Windsurf, Copilot, Cline, Codex, Kilo, Antigravity, Hermes, OpenCode, Pi) and hardened `generic/pre-tool-use.sh` with `exec condense "$@"` execution wrapper policy. Added first-class MCP client snippets and config paths via `condense mcp -c/--client <name>` and `condense mcp --list-clients` supporting 10 major agent clients. Hook installer supports non-destructive dry-run simulation (`condense init -n/--dry-run`), idempotent healing/updates (`condense init --update`), mid-run tampering detection against script truncation and removal, and isolated directory round trips. Added five new failure-contract catalog rows with native integration tests in `NativeHookIT` and `NativeMcpIT`. Updated [docs/HOOKS.md](docs/HOOKS.md) and [docs/mcp.md](docs/mcp.md) with dated compatibility matrices (Sep 2026).
- **Command-family breadth.** Fifty leftover catalog definitions cover the remaining verified zap families at pin `d9498bb` (ops, newer JS/Python, Ruby/Swift/Elixir/Quarto/Graphite/Jujutsu/Liquibase, and the rest of zap's TOML set). `CompetitiveInventoryTest` fails if a non-excluded zap family lacks a registered prefix. `NativeCatalogMatrixIT` PATH-stubs every `filters/index.toml` name. `sops` nonzero exits pass through raw. iptables keeps INPUT/FORWARD/OUTPUT headers with their rules and drops Docker/bridge sections. No new Java stage. Filter/IR stay at schema 1. See [docs/command-breadth.md](docs/command-breadth.md).
- **Structured .NET artifacts.** Leftover catalogs now cover `dotnet format`, `msbuild`, and `dotnet msbuild` as well as test/build/restore. Collision-safe sidecar injection adds `-bl:` or `--report` only when the user did not already name one. Condense never injects a TRX logger. `msbuild_binlog` reads v18–27 gzip records, `trx_report` is XXE-safe StAX, and `format_report` reads PascalCase JSON. Human console still falls through to grouping or the restore tail. Filter/IR stay at schema 1. Native proof is `NativeDotnetIT`. See [docs/dotnet-artifacts.md](docs/dotnet-artifacts.md).
- **Structured Terraform/OpenTofu parsing.** Leftover catalog definitions now cover terraform/tofu plan, apply, destroy, init, validate, fmt, and `state list`. `machine_ui` parses HashiCorp machine-UI NDJSON; `validate_json` parses a single validate object; `resource_graph` groups state addresses. Human text still falls through to grouping. Builtin `select_input = stdout_then_stderr` keeps a stderr `Error:` next to a JSON plan. Condense never injects `-json` and does not claim `state show`. Filter/IR stay at schema 1. Native proof is `NativeTerraformIT`. See [docs/machine-output.md](docs/machine-output.md).
- **Generated stage registry.** `@DeclarativeStage` plus an in-repo annotation processor emit `GeneratedStageRegistry`. Adding a stage no longer edits `StageFactory`. Schema numbers stay at filter/IR 1 and SQLite 2. Native proof is `NativeStageRegistryIT`. See [docs/schema-lifecycle.md](docs/schema-lifecycle.md) and [docs/generated/stage-inventory.md](docs/generated/stage-inventory.md).
- **Durable-state chaos harness.** `CondenseClock`, `DurableIo`, `AtomicFile`, `OrphanSweep`, and `durable-fault-contract.json` (every `id` maps to a `@Test` method). Native proof is `NativeChaosIT`. See [docs/durability.md](docs/durability.md).
- **Release budgets.** JVM pipeline and override benches now fail CI if relative overhead exceeds 100× the paired baseline (D29). Native Failsafe asserts an 80 MiB uncompressed ceiling and a per-OS median `--version` cold start (Linux 1500 ms, macOS 2500 ms, Windows 4000 ms). Soak, five-way concurrency, and analytics fail-open are Failsafe tests with isolated dirs. linux-x64 pushes to `main`, tag releases, and `phase3-verification.yml` run a 300-invocation soak. Releases checksum and cosign every blob including the CycloneDX SBOM. `RuntimeDependencyAllowlistTest` fails if a runtime Maven coordinate is added without updating the allowlist. See [docs/perf-baseline.md](docs/perf-baseline.md) and [SECURITY.md](SECURITY.md).
- **Adaptive proposals.** `condense propose` (and the MCP `propose` tool) emits schema-1 project override diffs from discover plus bounded analytics. `--write` creates `.condense/filters.toml.proposed` only; live `filters.toml` is never written. Coverage copies replay builtin `[[tests]]`. Proxy-path types do not import the propose package. Schema target stays 2. Native proof is `NativeProposeIT`. See [docs/propose.md](docs/propose.md).
- **Project discovery.** `condense discover` (and the MCP `discover` tool) recommends existing filter definition names from exact repository manifests and lockfiles. Rules are classpath `discover/*.toml` with an explicit per-family priority. `--root` may only narrow. Caps stop further probes and set `truncated`. Discovery does not change dispatch, hooks, or `filters.toml`. Native proof is `NativeDiscoverIT`. See [docs/discover.md](docs/discover.md).
- **Catalog leftover dispatch.** Builtin `filters/*.toml` `commands` that no Java `@CommandFilter` claims are registered on `CatalogBackedFilter`. Twenty-six leftover families ship as data only: mypy, dotnet test/build/restore/format, msbuild, bundle install, rspec, rubocop, terraform/tofu plan/apply/destroy/init/validate/fmt/`state list`, helm install/upgrade/list, gh/glab list commands, curl/wget/psql, pnpm install, next build, prisma migrate/generate, playwright, prettier. Optional builtin-only `select_input` (`stdout_then_stderr` on terraform/tofu plan-validate and the .NET build/test/format leftovers) and `[gate]` (curl uses `passthrough_nonzero_exit`). Native proof is `NativeCatalogIT` plus `NativeTerraformIT` and `NativeDotnetIT`. See [docs/filter-schema.md](docs/filter-schema.md).
- **Hook integrity, backups, and agent coverage.** Third-party agent configs are copied under `{dataDir}/backups/` before merge. Installed Condense scripts get a SHA-256 baseline; `condense init --show` and `condense doctor` report `ok` / `missing` / `tampered` / `unmanaged`. Claude Code now denies matching commands instead of rewrite-and-allow. Analytics schema target is 2 (`hook_events`, `hook_baselines`). New `--tool` values: Codex, OpenCode, Kilo, Antigravity, Hermes, Pi. Copilot, Windsurf, and Cline still install. MCP remains the preferred agent path. Native proof is `NativeHookIT`.
- **MCP server.** `condense mcp --start` speaks JSON-RPC 2.0 on stdio (no HTTP, no new Maven dependency). Tools are `run`, `explain`, `read`, `discover`, and `propose`. Resources are `condense://gain` and `condense://doctor`. `run` returns the schema-1 IR envelope; a non-zero child exit is not `isError`. MCP-supplied paths go through `ReadPathGate`. Bare `condense mcp` prints a client config snippet. Native proof is `NativeMcpIT`. See [docs/mcp.md](docs/mcp.md).
- **Structured diagnostics IR.** Filtered runs now attach a schema-1 `Document`. Default CLI stays compact text. `condense --format json` (options before the proxied command) prints one JSON object after the child exits. `condense explain --format json` embeds the same records. Exemplars are pytest (`test`), eslint (`diagnostic`), npm install (`dependency`), and docker ps (`resource`). Everyone else is `kind=opaque`. JSON never live-prints. Native proof is `NativeIrIT`. See [docs/ir.md](docs/ir.md).
- **Source-file reading.** `condense read FILE` opens a file with a per-language scanner (string, char, raw-string, and escape state) and returns verbatim, comment-stripped, or outline text. Line numbers map to the original file. JSON is a data language and never uses C-style comments. Unknown extensions stay verbatim. Workspace containment reuses `SafePathValidator`; `--root` can only narrow. Successful reads write a `gain` row. `condense cat` is unchanged. Native proof is `NativeReadIT`. See [docs/read.md](docs/read.md).
- **Incremental streaming.** Proxy mode can print filtered lines while a long command is still running. Mode is derived from each stage's declared streamability (`order_local` / `windowed` / `finalize_only` / `document`) — there is no `--stream` flag and no TOML `streamable` key. `npm install` and `docker build` emit irrevocable progress as it arrives, then the existing one-line success summary at exit. The proxy waits until the child exits unless `CONDENSE_COMMAND_TIMEOUT_SEC` is a positive integer. A 10 MB stream cap destroys the child and fail-opens instead of replacing the exit code. Timeout appends a diagnostic instead of replacing stderr. `condense explain` reports `pipeline_mode` and per-stage `streamability`. Native proof is `NativeStreamingIT` plus `NativeReliabilityIT`. See [docs/streaming.md](docs/streaming.md) and [docs/reliability.md](docs/reliability.md).
...
- **Persistence reliability.** Existing analytics databases are migrated with `PRAGMA user_version` (target 2). Every open enables WAL and `busy_timeout=5000`, prunes `commands`, `filter_outcomes`, `hook_events`, tee files, and hook-config backups older than 90 days, and records filter fail-open events. `condense doctor` (text and `--format json`) names why `gain` is empty and reports hook integrity. Native proof is `NativePersistenceIT` migrating a runtime-created v0 file inside the binary. See [docs/persistence.md](docs/persistence.md).
