...
- **Catalog leftover dispatch.** Builtin `filters/*.toml` `commands` that no Java `@CommandFilter` claims are registered on `CatalogBackedFilter`. Twenty-four leftover families ship as data only: mypy, dotnet test/build/restore, bundle install, rspec, rubocop, terraform/tofu plan/apply/destroy/init/validate/fmt/`state list`, helm install/upgrade/list, gh/glab list commands, curl/wget/psql, pnpm install, next build, prisma migrate/generate, playwright, prettier. Optional builtin-only `select_input` (`stdout_then_stderr` on terraform/tofu plan-validate) and `[gate]` (curl uses `passthrough_nonzero_exit`). Native proof is `NativeCatalogIT` plus `NativeTerraformIT`. See [docs/filter-schema.md](docs/filter-schema.md).
