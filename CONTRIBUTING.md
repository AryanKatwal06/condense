...
Native integration tests (`NativeCliIT`, `NativeAnalyticsIT`, `NativeCorpusIT`, `NativeBuiltinDefinitionIT`, `NativeTrustIT`, `NativePersistenceIT`, `NativeExplainIT`, `NativeStreamingIT`, `NativeReadIT`, `NativeIrIT`, `NativeMcpIT`, `NativeDiscoverIT`, `NativeProposeIT`, `NativeHookIT`, `NativeCatalogIT`, `NativeBudgetIT`, `NativeSoakIT`, `NativeConcurrencyIT`, `NativeAnalyticsFailOpenIT`) require `native.image.path` and **fail rather than skip** if the binary is missing. They set `CONDENSE_CONFIG_DIR` and `CONDENSE_DATA_DIR` on the child process so they never write the developer's real analytics database. Project `.condense/filters.toml` is skipped until `condense config trust` (or a CI hatch with a listed CI indicator). See [docs/trust.md](docs/trust.md). Filtered `FilterResult.of` output starts with `condense[filtered]`. Persistence schema, WAL, retention, and `condense doctor` are in [docs/persistence.md](docs/persistence.md). Per-stage explainability is `condense explain` — see [docs/explain.md](docs/explain.md). Source-file reading is `condense read` — see [docs/read.md](docs/read.md). Project discovery is `condense discover` — see [docs/discover.md](docs/discover.md). The stdio MCP server is `condense mcp --start` — see [docs/mcp.md](docs/mcp.md). Hooks are the fallback; see [docs/HOOKS.md](docs/HOOKS.md).

See [docs/perf-baseline.md](docs/perf-baseline.md) for the CI gates (relative JVM overhead, native size, native cold-start median, soak leak ratio). Token estimates are documented in [docs/token-estimator.md](docs/token-estimator.md); `TokenEstimatorAccuracyTest` fails `mvn test` if p95 error vs cl100k_base exceeds the published bound. Filter fidelity is documented in [docs/fidelity-corpus.md](docs/fidelity-corpus.md); `FidelityCorpusTest` fails if a catalogued critical signal is dropped or a baked savings floor is missed. Adding a runtime Maven dependency also requires updating `src/test/resources/supplychain/runtime-dependencies.txt`.
