...
condense --version / --help
  ...
  CondenseRootCommand (picocli @Command)
        ...
        └── McpCommand --start → hand-rolled stdio JSON-RPC (tools run/explain/read/discover/propose, resources gain/doctor)
```
...
| `ProposeCommand.java` | `com.condense.propose` | `condense propose` — reviewable project override diffs; never writes `filters.toml` |
...
9. **Three-tier filter composition**: Project `.condense/filters.toml` (TOFU + capability ceiling) then user-global `filters.toml` (trusted by location) then builtin `classpath:filters/<name>.toml` via `BuiltinDefinitionCatalog` (fail-closed). Enumeration is `filters/index.toml` — never a classpath directory walk. After CDI beans, `StrategyRegistry` registers leftover catalog `commands` on `CatalogBackedFilter`. `StageFactory` is a facade over the compile-time `GeneratedStageRegistry`. Schema v1 requires `schema_version = 1` and rejects unknown keys. Builtin-only optional keys are `select_input` and `[gate]`. See `docs/filter-schema.md` and `docs/trust.md`.
...
13. **MCP over stdio**: `condense mcp --start` is a hand-rolled JSON-RPC server (no extra Maven dependency). `run` returns the Phase 11 envelope; `explain` / `read` / `discover` / `propose` / `gain` / `doctor` reuse existing records. MCP paths go through `ReadPathGate` / the same narrow-only root as `condense read`. Logs go to stderr so stdout stays JSON-RPC-only. MCP is the preferred agent path; hooks are the fallback. See `docs/mcp.md`.
...
16. **Reviewable adaptive proposals**: `condense propose` emits schema-1 project override diffs from discover plus bounded analytics reads. `--write` creates `.condense/filters.toml.proposed` only. Proxy-path types do not import `com.condense.propose`. Schema target stays 2. See `docs/propose.md`.
