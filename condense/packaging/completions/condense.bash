# Bash completion for condense
# Install: source /path/to/condense.bash
#          or: cp condense.bash /etc/bash_completion.d/condense

_condense_completion() {
  local cur prev words
  COMPREPLY=()
  cur="${COMP_WORDS[COMP_CWORD]}"
  prev="${COMP_WORDS[COMP_CWORD-1]}"

  # Top-level subcommands
  local subcommands="gain doctor discover propose explain read init config mcp"

  # Commands condense can proxy
  local proxy_commands="git cargo pytest go npm npx docker kubectl aws ls grep rg find cat make mvn gradle vitest jest eslint tsc ruff pip golangci-lint python python3 mypy dotnet bundle rspec rubocop terraform helm gh glab curl wget psql pnpm next prisma playwright prettier"

  # Top-level options
  local global_opts="-v --verbose -u --ultra-compact --format --version --help"

  case "$prev" in
    condense)
      COMPREPLY=($(compgen -W "${subcommands} ${proxy_commands} ${global_opts}" -- "$cur"))
      return 0
      ;;
    gain)
      COMPREPLY=($(compgen -W "--graph --history --scope --daily --weekly --top --since --all --format --help" -- "$cur"))
      return 0
      ;;
    doctor)
      COMPREPLY=($(compgen -W "--format --help" -- "$cur"))
      return 0
      ;;
    discover)
      COMPREPLY=($(compgen -W "--format --root --help" -- "$cur"))
      return 0
      ;;
    propose)
      COMPREPLY=($(compgen -W "--format --root --write --help" -- "$cur"))
      return 0
      ;;
    explain)
      COMPREPLY=($(compgen -W "--format --input --stdin --exit-code --dropped-limit --help" -- "$cur"))
      return 0
      ;;
    read)
      COMPREPLY=($(compgen -W "--level --lang --root --max-bytes --format --stdin --ultra-compact --help" -- "$cur"))
      return 0
      ;;
    init)
      COMPREPLY=($(compgen -W "-g --global --show --remove --tool --help" -- "$cur"))
      return 0
      ;;
    config)
      COMPREPLY=($(compgen -W "--list --get --set --reset --help" -- "$cur"))
      return 0
      ;;
    mcp)
      COMPREPLY=($(compgen -W "--start --help" -- "$cur"))
      return 0
      ;;
    --scope)
      COMPREPLY=($(compgen -W "global project" -- "$cur"))
      return 0
      ;;
    --format)
      COMPREPLY=($(compgen -W "text json" -- "$cur"))
      return 0
      ;;
    --tool)
      COMPREPLY=($(compgen -W "claude-code cursor gemini windsurf copilot cline codex opencode kilo antigravity hermes pi" -- "$cur"))
      return 0
      ;;
    --get|--set)
      COMPREPLY=($(compgen -W "tee.enabled tee.mode hooks.exclude_commands" -- "$cur"))
      return 0
      ;;
  esac

  COMPREPLY=($(compgen -W "${subcommands} ${proxy_commands} ${global_opts}" -- "$cur"))
  return 0
}

complete -F _condense_completion condense
